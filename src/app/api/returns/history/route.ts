...
// GET /api/returns/history - Fetch all return transactions with optional search
export async function GET(request: NextRequest) {
  try {
    ...
    const whereClause = searchTerm && searchTerm.trim().length > 0
      ? {
        OR: [
          { customerName: { contains: searchTerm.trim(), mode: 'insensitive' as const } },
          { customerShopName: { contains: searchTerm.trim(), mode: 'insensitive' as const } },
          { originalSaleId: { contains: searchTerm.trim(), mode: 'insensitive' as const } },
        ],
      }
      : undefined;

    // Fetch returns with optional search filter
