...
const CATEGORIES = ["All", "Yogurt", "Drink", "Ice Cream", "Dessert", "Curd", "Other"] as const;
...
export default function SalesPage() {
  ...
  // OPTIMIZATION: Do not fetch all sales initially. Only fetch what's needed or paginate.
  const { sales: allSales, setSales: setAllSales } = useSalesData(false);
  
  // OPTIMIZATION: Server-side search
  const [selectedCategory, setSelectedCategory] = useState<string>("All");
  const [searchTerm, setSearchTerm] = useState("");
  const [debouncedSearchTerm, setDebouncedSearchTerm] = useState("");

  const { products: searchResults, isLoading: isSearchLoading, refetch: searchProducts } = useProductSearch(debouncedSearchTerm, selectedCategory);
  ...
  // Client-side filtering removed in favor of Server-side search logic above
  
  // Removed complex categories useMemo

  // OPTIMIZATION: State for outstanding balance, fetched from API instead of calculated from all 10k sales
  const [customerOutstandingBalance, setCustomerOutstandingBalance] = useState(0);

  // Removed expensive useMemo that depended on allSales
  // const customerOutstandingBalance = useMemo(() => { ... }, [selectedCustomer, allSales]);
    ...
    if (customer) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error('Failed to fetch customer financials.');
            }
            const data = await response.json();
            setCustomerCreditBalance(data.availableCredit || 0);
            setCustomerOutstandingBalance(data.outstandingBalance || 0);
        } catch (error) {
            console.error(error);
            setCustomerCreditBalance(0);
            setCustomerOutstandingBalance(0);
            toast({ variant: 'destructive', title: 'Error', description: 'Could not fetch customer balance.' });
        } finally {
  ...
  const handleToggleOffer = (checked: boolean) => {
    ...
    const sourceProducts = viewMode === 'vehicle' ? vehicleStock || [] : searchResults || [];
    setCartItems(prev => reconcileOfferItems(prev, checked, sourceProducts, newExclusions));
  };
  ...
  const productsForDisplay = viewMode === 'vehicle' ? vehicleStock : searchResults;
  ...
  const handleAddToCart = (productToAdd: Product) => {
    ...
    setCartItems(prevItems => {
      ...
      return reconcileOfferItems(updatedCart, isBuy12Get1FreeActive, searchResults, excludedOfferProductIds);
    });
  ...
  const handleUpdateQuantity = (productId: string, quantity: number, saleType: 'retail' | 'wholesale') => {
    // If in vehicle mode, we use vehicle stock. If main mode, we use search results.
    // Note: searchResults only contains currently searched items. 
    const sourceProducts = viewMode === 'vehicle' ? vehicleStock : searchResults;
    
    const productInStock = sourceProducts?.find(p => p.id === productId);
    
    // If product is found in current view (search results/vehicle), enforce stock limit.
    // If NOT found (e.g. user cleared search), we cannot enforce limit client-side easily without fetching.
    // We allow the update, assuming the user knows what they are doing. Server will reject if stock is low.
    const stockLimit = productInStock ? productInStock.stock : 999999;
  
    const targetQuantity = Math.max(0, Math.min(quantity, stockLimit));
    ...
    setCartItems(prevItems => {
      ...
      // Pass sourceProducts (which might be empty/partial) to offer reconciliation. 
      // If product not found, offers for it might be removed/not calculated. This is a known limitation of server-side search.
      return reconcileOfferItems(updatedCart, isBuy12Get1FreeActive, sourceProducts || [], excludedOfferProductIds);
    });
  };
  ...
  const handleRemoveItem = (productId: string, saleType: 'retail' | 'wholesale', isOfferItem: boolean) => {
    ...
    setCartItems(prevItems => {
        ...
        return reconcileOfferItems(updatedCart, isBuy12Get1FreeActive, searchResults, newExclusions);
    });
  };
  ...
  const currentSubtotal = useMemo(() => {
    return cartItems.filter(item => !item.isOfferItem).reduce((sum, item) => {
      // Use appliedPrice directly from the cart item which is set at add time
      return sum + (item.appliedPrice * item.quantity);
    }, 0);
  }, [cartItems]);
  ...
  if (isSearchLoading && !searchResults?.length && !searchTerm) { 
    return (
  ...
  // Removed generic productsError check here as error is handled gracefully in UI or toast
      ...
      <div className={cn(
        ...
        <div className="flex-1 lg:w-2/3 flex flex-col min-h-0">
          ...
          <ScrollArea className="flex-1 p-3 sm:p-4 bg-white dark:bg-transparent">
            { (isSearchLoading || isVehicleStockLoading) ? (
